...
```
数据结构_栈:先进后出 (类似于弹夹)
数据结构_队列:先进先出 (类似于吃饭拉屎)
数据结构_数组:查询快,增删慢(数组的地址是连续的,我们可以通过数组的首地址可以找到数组,通过数组索引可以快速查找某个元素),(数组的长度是固定的,我们想要增加/删除一个元素,必须创建一个新数组,吧原数组的内容复制过来)
数据结构_链表:查询慢,增删快(链表中的地址不是连续的,每次查询元素都要从头开始查询),(链表结构增加/删除一个元素对链表的整体结构没有影响,所以增删快 )
    单项链表:链表中只要一条链子,不能保证元素的顺序(存储元素和取出元素的顺序有可能不一致)
    双向链表:链表中有两条链子,有一条专门记录元素顺序是一个有序集合
(二叉树,排序树/查找树,平衡树,不平衡树)
数据结构_红黑树:趋近于平衡树 查询速度非常快,查询叶子节点最大次数和最小次数不能超过2倍
    约束:
        1.节点可以是红色的或者黑色的
        2.根节点是黑色的
        3.叶子节点(空节点)是黑色的
        4.每个红色节点的子节点都是黑色的
        5.任何一个节点到每个叶子节点的所有路径上的黑色节点数相同
                黑
            红        红 
        黑      黑  黑     黑
```
```
线程:
    并发:指两个或者多个事件在同一时间段发送 <交替执行>
    并行:指两个或者多个事件在同一时刻发生(同时发生)<同时执行>
线程调度 :
    分时调度:所有线程轮流使用cpu的使用权,平均分配每个线程占用cpu的时间
    抢占式调度:优先让线程优先级高的线程使用cpu,如果优先级相同 随机使用(线程随机性) java就是抢占式调度
    抢占式调度:优先让线程优先级高的线程使用cpu,如果优先级相同 随机使用(线程随机性) java就是抢占式调度
```